...
| `custom-user-selection.md` | `run.py` | `06_distill.py` | Optional category selection — uncheck to exclude from distillation |
...
### Category Selection (`custom-user-selection.md`)

**Purpose:** Let the user review and refine which categories get distilled, after the automated pipeline has run.

**Generated by:** `run.py` — created automatically after step 5 completes.
...
- [x] docker-compose (12 conversations, 187 messages)
- [x] excel-pivot (8 conversations, 94 messages)
- [ ] general-chat (3 conversations, 15 messages)
- [x] recipe-calorie (6 conversations, 72 messages)
```

**How it works:**
- All categories start checked `[x]` — no action needed if you want everything distilled
- Uncheck a category by changing `[x]` to `[ ]` to exclude it from distillation
- `06_distill.py --status` shows excluded categories as `✗ excluded`
- If the file is deleted or absent, all categories are distilled (backward compatible)
- The file has no effect on steps 1–5 — it only controls distillation (step 6)

**When to use:**
- After reviewing `CHECKLIST.md` or the extracts, you decide some categories aren't worth distilling
- After a first distillation pass, you want to re-run without certain low-value topics
- You want to focus distillation on a subset of categories to control token costs
...
**Modes:**
- `--status` — Show distillation progress (pending/done/excluded per category)
...
└── myrun/                  # Example run directory
    ...
    ├── CHECKLIST.md
    ├── custom-user-selection.md  # Category selection (generated, user-editable)
...
# → also generates custom-user-selection.md with all categories checked
python run.py --config myrun/config.json

# (Optional) User reviews custom-user-selection.md and unchecks categories
# to exclude from distillation. If skipped, all categories are distilled.

# Step 6: Distill extracts into knowledge files (AI inference)
# Respects custom-user-selection.md — only distills checked categories
# Claude Code reads each selected category_extract and writes learning-*.md files
